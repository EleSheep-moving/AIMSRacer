...
8. [Use system Python](#8-use-system-python)
...
See the [MPCC setup and validation limits](../src/controller/docs/usage.md#prepare-the-real-car)
...
sudo install -D -m 0644 \
  systemd/jetson-clocks.service \
  /etc/systemd/system/jetson-clocks.service
sudo systemctl daemon-reload
sudo systemctl enable --now jetson-clocks.service
```

Verify that the command ran successfully and that the service is enabled for
future boots:

```bash
systemctl status jetson-clocks.service --no-pager
systemctl is-enabled jetson-clocks.service
systemctl is-active jetson-clocks.service
```

If the service is no longer wanted, disable it and remove the installed unit:

```bash
sudo systemctl disable --now jetson-clocks.service
sudo rm /etc/systemd/system/jetson-clocks.service
sudo systemctl daemon-reload
```
...
See [vehicle bringup](operations/bringup.md) for vehicle operation, mapping, localization, and
