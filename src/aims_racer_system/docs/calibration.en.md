...
[中文](calibration.md) · [Vehicle bringup](../../../docs/operations/bringup.md)

PP workflows default to `/odometry/filtered`; non-PP workflows default to
`/rear_axle/wheel_odom`. A supplied `calib_config.yaml` or parameter override can
change these defaults; check the effective `odom_topic`.
...
## 1) Localization-based: Pure Pursuit Auto Loop Calibration (Defaults to `/odometry/filtered`)

### 1.1 When to Use

- You have stable `nav_msgs/Odometry` (Pure Pursuit defaults to EKF `/odometry/filtered`; `/rear_axle/wheel_odom` is wheel odometry, not fused localization).
...
Subscribes:
- `/odometry/filtered` (`nav_msgs/Odometry`, default PP localization input)
- `/sensors/core` (`vesc_msgs/VescStateStamped`, telemetry)
...
ros2 bag record -o pp_calib \
   /odometry/filtered /rear_axle/wheel_odom \
   /sensors/core \
   /calib/ackermann_cmd \
...
python3 src/aims_racer_system/scripts/longitudinal_model_verify.py \
   --base speed_hold_current_results.txt \
   --bags /data/pp_accel_interval1 /data/pp_accel_interval2 \
   --valid-v-end 6.0 \
   --model-accel-max 2.5 \
