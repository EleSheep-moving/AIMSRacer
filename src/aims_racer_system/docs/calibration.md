...
[English](calibration.en.md) · [整车启动](../../../docs/operations/bringup.md)

PP 工作流默认使用 `/odometry/filtered`；非 PP 工作流默认使用 `/rear_axle/wheel_odom`。
加载 `calib_config.yaml` 或传入参数可能覆盖默认值，请检查实际 `odom_topic`。
...
## 1) 有定位方法：Pure Pursuit 自动跑圈标定（默认使用 /odometry/filtered）

### 1.1 适用场景

- 能稳定提供 `nav_msgs/Odometry`（Pure Pursuit 默认使用 EKF 的 `/odometry/filtered`；`/rear_axle/wheel_odom` 仅为轮速里程计，不是融合定位）。
...
订阅：
- `/odometry/filtered`（`nav_msgs/Odometry`，PP 默认定位输入）
- `/sensors/core`（`vesc_msgs/VescStateStamped`，用于 ERPM/遥测）
